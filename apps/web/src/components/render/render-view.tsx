...
export function RenderView({
  ...
  /**
   * The countdown leader is a lovely thing to watch and says nothing out loud.
   * Announce the film reaching each tenth and then the verdict — a render runs
   * for minutes, so anything finer would be a monologue.
   */
    ...
    useCallback(
      (payload: RenderProgressPayload) => {
        ...
        if (payload.status === "done") {
          setAnnouncement("The film is printed. It's ready to watch.");
        } else if (payload.status === "failed") {
          setAnnouncement("The print failed. Nothing is lost — have a look at what went wrong.");
        } else {
          const milestone = Math.floor((payload.progress ?? 0) / 10) * 10;
          if (milestone > lastMilestone.current) {
            lastMilestone.current = milestone;
            setAnnouncement(`Printing the film — ${milestone} per cent`);
          }
        }
  ...
  return (
    <>
      {/*
        Outside the three screens rather than in each of them: done and failed
        swap the whole view in the same render that writes the message, and a
        live region that mounts with its text already in it says nothing.
      */}
  ...
  function screen() {
          ...
          <div className="mt-5 grid gap-5 lg:grid-cols-[1fr_240px]">
            <div>
              <div className="border border-[color:var(--hair-dark)] bg-black p-1.5">
                {/* Width auto rather than full: an upright film is twice as
                    tall as it is wide, and the screening room shouldn't make
                    anyone scroll to find the controls. */}
                <video
                  src={publishedRender.url}
                  controls
                  playsInline
                  className="mx-auto block max-h-[72vh] w-auto max-w-full"
                />
              </div>
              ...
              <div className="mt-3 flex flex-wrap items-center gap-2">
                ...
                <span className="font-mono text-2xs uppercase tracking-label text-ink-400 sm:ml-auto">
            ...
            <aside className="border border-[color:var(--hair-dark)] bg-ink-850 px-3 py-3">
              ...
              <p className="mt-3 font-mono text-2xs leading-relaxed text-ink-400">
      ...
      return (
        <div className="mx-auto max-w-xl py-10">
          ...
          <p className="mt-2 text-[13px] leading-relaxed text-ink-300">
            Nothing is lost — the cut, the marks and the footage are all still there.
            {isCreator
              ? " Have a look at what the renderer said, then send it again."
              : " Whoever started this vlog can send it to the lab again."}
          </p>
          {error && (
          ...
          {/*
            A second attempt from right here, rather than "go to the bench and
            find the button again": a failed print leaves the film exactly as it
            was, so trying once more is the obvious next move and used to be the
            one thing this screen wouldn't let anyone do.
          */}
          {isCreator && (
            <div className="mt-4 flex flex-wrap items-center gap-2">
              <PrintFilmButton
                slug={slug}
                label="Try the print again"
                disabled={clips === 0}
    ...
    return (
      <div className="mx-auto max-w-xl py-6 text-center sm:py-8">
        ...
        <p className="mt-5 font-mono text-2xs uppercase tracking-label text-ink-400">
